...
def agendar_cita():
    if 'loggedin' in session and session.get('role') == 'user':
        ...
        if request.method == 'POST':
            ...
            if numero_orden:
                cursor.execute('SELECT numero_orden FROM ordenes WHERE numero_orden = %s AND estado = "Activa"', (numero_orden,))
    ...
    if 'loggedin' in session and session.get('role') == 'user':
        ...
        if cita and cita[0] == 'Disponible':
            ...
            if especialidad and especialidad[0] != "Medicina General":
                ...
                cursor.execute('SELECT id FROM ordenes WHERE cliente_id = %s AND estado = "Activa"', (cliente_id,))
                orden = cursor.fetchone()
                if not orden:
                    conn.close()
                ...
                # Cambiar el estado de la orden a "Usada"
                cursor.execute('''
                    UPDATE ordenes
                    SET estado = 'Usada'
                    WHERE id = %s
...
def mis_ordenes():
    if 'loggedin' in session and session.get('role') == 'user':
        ...
        # Obtener las órdenes activas del usuario actual
        cursor.execute('''
            SELECT o.numero_orden, 
                   CASE 
                       WHEN LEFT(o.numero_orden, 2) = '01' THEN 'Cardiología'
                       WHEN LEFT(o.numero_orden, 2) = '02' THEN 'Pediatría'
                       WHEN LEFT(o.numero_orden, 2) = '03' THEN 'Ginecología'
                       WHEN LEFT(o.numero_orden, 2) = '04' THEN 'Neurología'
                       WHEN LEFT(o.numero_orden, 2) = '05' THEN 'Psiquiatría'
                       ELSE 'Especialidad Desconocida'
                   END AS especialidad
            FROM ordenes o
            JOIN usuarios u ON o.cliente_id = u.cliente_id
            WHERE u.username = %s AND o.estado = 'Activa'
        ''', (session['username'],))
        ordenes_activas = cursor.fetchall()

        # Obtener las órdenes usadas del usuario actual
        cursor.execute('''
            SELECT o.numero_orden, 
                   CASE 
                       WHEN LEFT(o.numero_orden, 2) = '01' THEN 'Cardiología'
            ...
            FROM ordenes o
            JOIN usuarios u ON o.cliente_id = u.cliente_id
            WHERE u.username = %s AND o.estado = 'Usada'
        ''', (session['username'],))
        ordenes_usadas = cursor.fetchall()

        conn.close()

        # Pasar las órdenes activas y usadas a la plantilla
        return render_template('mis_ordenes.html', ordenes_activas=ordenes_activas, ordenes_usadas=ordenes_usadas)
    return redirect(url_for('login'))
...
def asignar_orden(cita_id):
    if 'loggedin' in session and session.get('role') == 'doctor':
        ...
        if request.method == 'POST':
            ...
            cursor.execute('''
                INSERT INTO ordenes (cliente_id, numero_orden, descripcion)
                VALUES (%s, %s, %s)
            ''', (paciente[0], numero_orden, f"Orden para {especialidad}"))
