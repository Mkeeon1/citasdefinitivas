...
def init_db():
    ...
    # Verificar si la columna numero_orden ya existe en la tabla ordenes
    cursor.execute('''
        SELECT COUNT(*)
        FROM INFORMATION_SCHEMA.COLUMNS
        WHERE TABLE_NAME = 'ordenes' AND COLUMN_NAME = 'numero_orden'
    ''')
    if cursor.fetchone()[0] == 0:
        # Agregar la columna numero_orden a la tabla ordenes
        cursor.execute('''
            ALTER TABLE ordenes
            ADD COLUMN numero_orden VARCHAR(50) UNIQUE NOT NULL
        ''')

    # Verificar si la columna estado ya existe en la tabla ordenes
    cursor.execute('''
        SELECT COUNT(*)
        FROM INFORMATION_SCHEMA.COLUMNS
    ...
    ''')
    if cursor.fetchone()[0] == 0:
        # Agregar la columna estado a la tabla ordenes
        cursor.execute('''
            ALTER TABLE ordenes
